...
> **Related work and priority.** XllentAI's immutable historical [`modular_arithmetic` snapshot](https://huggingface.co/XllentAI/modular_arithmetic/tree/3d2c226c2382140b890026bdfdd59485daa192ba) predates NeuralHorner's first public GitHub commit. It already learned the same modulus-conditioned Horner/double-and-add transition with hard binary state and reported held-out-prime transfer. Its [current snapshot as of 2026-08-01](https://huggingface.co/XllentAI/modular_arithmetic/tree/f704813dc64d7c04cc9ba7dbf9a3a281c431628d) reports two shared carry-aware TCN weight sets (~10.7M parameters) spanning tiers 1-10. We claim neither the Horner recurrence, hard binary state, modulus conditioning, nor cross-prime transfer. NeuralHorner's narrower contribution is one compact ~471K-parameter BiGRU checkpoint reused for learned operand reductions and multiplication, with no Python-side operand `% p`, plus challenge-width efficiency and exactness/failure/causal diagnostics.
