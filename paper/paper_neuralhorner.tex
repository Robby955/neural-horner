...
\begin{center}
\textit{Draft in progress, last updated 2026-06-28 23:17 UTC}
\end{center}
...
modular reduction; the cell scores $40/100$ there, failing on large operands. The learned
...
\begin{table}[t]
 ...
 \bottomrule
 \end{tabular}
 \caption{Per-tier exact-match on the official scorer (single rented H100), for each of
...
The official scorer runs eleven tiers, $\mathrm{T0}$ through $\mathrm{T10}$, at
$100$ problems each ($1100$ in total), distinct from the separate
$64$-case-per-tier weight-perturbation battery used for the anti-cheat check
(Table~\ref{tab:tiers}). $\mathrm{T0}$ is a plain-multiplication diagnostic with no
modular reduction; $\mathrm{T1}$--$\mathrm{T10}$ are the scored modular tiers. The
model clears all ten scored tiers at $100/100$ each (overall accuracy $100\%$ on the
ranked scorer, $1000/1000$), while the $\mathrm{T0}$ diagnostic scores $40/100$, so
the raw all-tier count is $1040/1100$. On the scorer counts, exact
(Clopper--Pearson) $95\%$ confidence intervals are: each scored tier ($100/100$) is
$[0.964, 1.000]$, the ten scored modular tiers pooled ($1000/1000$) is $[0.9963,
1.000]$, the held-out battery ($759/768$) is $[0.978, 0.995]$, the Fermat family
($119/128$) is $[0.871, 0.967]$, and the $\mathrm{T0}$ plain-multiplication
diagnostic ($40/100$) is $[0.303, 0.503]$. The failures are not spread across the Fermat
...
Scoring uses the challenge's official open-source scorer, run on a single H100 rented via RunPod. Per-tier exact-match is reported over three independent seeds
...
\begin{verbatim}
modchallenge evaluate <submission> --total 1100 # T0-T10, 100/tier
python scripts/verify_no_shortcut.py <submission> --randomize # 0/64/tier
