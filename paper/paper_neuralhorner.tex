...
% TITLE
% ============================================================
\title{NeuralHorner: One Modulus-Conditioned Cell Clears All Ten Tiers
of the Modular Arithmetic Challenge}
...
per-step state width to each prime's bit-length (dynamic-$L$); this is exact, the
dropped bits being always zero, and it is what brings the run under budget.
...
answer-preserving rather than approximate. The scorer batches problems within a
...
$40954$ state-and-prime combinations for primes below $64$, checked exhaustively. Against a per-step-independent null, a rollout that is correct over the $k\sim10^4$ steps of the deep tiers requires a per-step error below about $10^{-4}$ (since accuracy $\approx(1-\epsilon)^k$); the exhaustive zero-error per-step result and the $1.00$ rollout are consistent with a per-step error below the rollout's resolution.
...
per-step state width to each prime's bit-length is answer-preserving (dropped bits
...
transitions matched ($40954/40954$). This certifies the single-step map; it does
...
(Section~\ref{sec:method:output}) confirms that every solved tier collapses from
...
Reporting these as rates alone understates the picture, so we give exact
(Clopper--Pearson) $95\%$ confidence intervals: each cleared tier ($100/100$) is
...
\begin{table}[t]
  ...
  \begin{tabular}{lccc}
    \toprule
    Held-out family & Cases & Exact & Rate \\
    \midrule
...
of matched shape and confirms that every tier collapses from $64/64$ to $0/64$.
