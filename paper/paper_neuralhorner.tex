...
observation here is that the wall tracks the monolithic framing rather than
...
carry decision near the state boundary rather than a diffuse loss of accuracy. We
offer a mechanism as a hypothesis, not a verified claim: $F_{11}=2^{2048}+1$ is two
set bits separated by $2047$ zeros, so the cell must carry the modular state across
$2047$ near-identical zero-input steps, where small drift accumulates before the
final set bit arrives --- the long-homogeneous-trajectory failure \citet{price2016neural}
report for the Neural GPU. Targeted training on this structured slice, in the spirit
of the custom modular-arithmetic distributions of \citet{saxena2024modarith}, is the
natural fix.
...
A reader should not read $1.00$ on the scorer as exactness. A candidate remedy,
untested here, is an embedding that respects the cyclic modular boundary, as used in
transformer attacks on lattice cryptography \citep{salsafresca2024}, together with
counterexample-driven training on the failing family.
...
This entry is the most-scaffolded point of a longer study, not its endpoint. The
schedule is fixed here; the natural arc hands it back to the network in stages --- a
learned controller over the same transition, a learned latent state in place of
explicit residue bits, then a recurrent bit processor, a looped transformer, and a
monolithic model --- and measures where cross-prime transfer breaks. That ladder
turns the present result into a quantitative question about how much algorithmic
structure must be fixed before learned modular arithmetic generalizes, the
algorithmic-alignment trade-off \citep{velickovic2021nar, bohde2024markov}.
