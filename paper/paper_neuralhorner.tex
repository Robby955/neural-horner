...
carry decision near the state boundary rather than a diffuse loss of accuracy. Trace
certification localizes it exactly: on $F_{11}=2^{2048}+1$, $5$ of $8$ cases fail, and
in every failing case the first and only divergence is the last step of reducing $a$,
at state $s=2^{2047}$. The $2048$ preceding doublings are exact, so this is not
cumulative drift but a single undertrained reduction --- the largest modular wrap,
$(2\cdot 2^{2047}+1)\bmod p = (2^{2048}+1)\bmod p$, which uniform training rarely
samples. This matches the long-carry brittleness \citet{price2016neural} report for the
Neural GPU; targeted training on the extreme-boundary reduction, in the spirit of the
custom modular-arithmetic distributions of \citet{saxena2024modarith}, is the natural
fix.
