...
\begin{center}
\textit{Draft in progress, last updated 2026-06-28 16:40 UTC}
\end{center}
...
through~$10$ at exact-match $1.00$ on the official open-source scorer (run locally,
...
receipts (exhaustive over all $40954$ states of every prime below $64$), a
...
Outside the scored tiers, the scorer reports a Tier-0 diagnostic that isolates
multiplication from modular reduction. The cell scores $40/100$ there, failing on the
large operands (where it returns $0$). This is informative rather than incidental: the
learned computation realizes $(a\,b)\bmod p$ inside the reduce-reduce-multiply regime it
is trained in, and does not behave as a general large-integer multiplier in the
no-reduction regime the diagnostic probes. We therefore scope the claim to modular
multiplication on the scored distribution, not pure multiplication, and read the Tier-0
gap as the same class of distribution-specific brittleness as the power-of-two-adjacent
residual: the cell is exact where it is exercised and degrades off that distribution.
...
\begin{table}[t]
 ...
 \bottomrule
 \end{tabular}
 \caption{Three scorer-operand seeds of the single trained checkpoint on the official scorer (single H100). Each run
...
pure-multiplication diagnostic ($40/100$, Section~\ref{sec:results:tiers}) is a second,
